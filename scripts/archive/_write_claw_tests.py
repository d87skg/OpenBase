...
class TestOpenClawAgent:
    def test_agent_run_task(self):
        ...
        assert len(result.events) >= 2  # agent_started + tool_call + tool_result + agent_finished
    ...
    def test_agent_trust_score(self):
        ...
        result = agent.run("shell echo step1\nshell echo step2\nshell echo step3")
    ...
    def test_agent_list_tools(self):
        ...
        assert len(tools) == 3  # read_file, write_file, shell
...
path = r'D:\OpenBase\openclaw\tests\test_agent.py'
os.makedirs(os.path.dirname(path), exist_ok=True)
with open(path, 'w', encoding='utf-8') as f:
    f.write(code)
print('test_agent.py written')
